...
| **MTranServer**     | `mtranserver` | `MTRANSERVER_ENDPOINT`, `MTRANSERVER_API_TOKEN`                       | `http://127.0.0.1:8989`, `[Your Token]`              | [MTranServer](https://github.com/xxnuo/MTranServer) 참조, CPU에서 실행되는 오프라인 번역 서버입니다. 서버에 `MT_API_TOKEN`이 설정된 경우에만 `MTRANSERVER_API_TOKEN`이 필요합니다.                                                                            |
